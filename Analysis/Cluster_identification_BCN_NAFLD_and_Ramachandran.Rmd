---
title: "Cluster identification - BCN NAFLD and Ramachandran"
...
```{r}
load(file.path(data_storage_folder, "BCN_NAFLD_and_Ramachandran_CD45_PBMC.combined.RData"))
...
DimPlot(scData.combined, reduction = "umap", label = TRUE, split.by = "sample_label", ncol = 4)
```

## Relation between new clusters and clusters in BCN-NAFLD-only analysis

```{r}
BCN_NAFLD_meta = read.table("./Metadata_complete.txt", sep="\t", header=T, stringsAsFactors = F)
rownames(BCN_NAFLD_meta) = BCN_NAFLD_meta$orig_cell_id
...
```{r}
tt = table(scData.combined$seurat_clusters, scData.combined$clusters_BCN_NAFLD, dnn=c("New clusters", "BCN NAFLD cluster labels"))
tt
```
...
if(rerun_analyses) {
  ...
  save(cluster_markers, file=file.path(data_storage_folder, "All_markers_BCN_and_Ramachandran.Rdata"))
  write_xlsx(cluster_markers,file.path(result_folder, "All_markers_BCN_and_Ramachandran.xlsx"))
} else {
  load(file.path(data_storage_folder, "All_markers_BCN_and_Ramachandran.Rdata"))
}
```
...
DefaultAssay(scData.combined) <- "RNA"
  ...
  save(cons_markers, file=file.path(data_storage_folder,"Conserved_cluster_markers_BCN_and_Ramachandran.Rdata"))
  ...
  write_xlsx(cons_markers_all,file.path(result_folder, "All_cons_markers_BCN_and_Ramachandran.xlsx"))
  
} else {
  load(file.path(data_storage_folder,"Conserved_cluster_markers_BCN_and_Ramachandran.Rdata"))
}
...
compute_functional_enrichment <- function(sc_data, curr_dge_tab, title) {
  ...
  names(tmp2) = unlist(lapply(tmp2, function(x) unique(as.character(x$Cluster))))
...
project_name = "BCN_and_Ramachandran"
...
sel_markers=toupper(c("Ptprc","Pglyrp1","Cd4","Cd3e","Cd3d","Cd3g", "Gata3","Cxcr6","Rora","Cd7","Cd8a","Cd19","Ms4a1","Ly6a", "Cd34","Procr","Fgd5","Hoxb5","Epor","Slamf1","EPCAM","VIM"))
DefaultAssay(scData.combined) <- "RNA"
...
```{r}
cluster_assignment = read.table("Cluster_assignments/BCN_NAFLD_Clusters_with_Ramachandran_2022-01-10.txt", sep="\t",header=T, stringsAsFactors = F)
options(width = 180)
...
```{r}
save(scData.combined,file=file.path(data_storage_folder, "BCN_NAFLD_Ramachandran_CD45_MoMF_DC_only.combined_cluster_annotated.RData"))
```


# Export

Cluster assignations have been exported to file *Metadata_complete_Ramachandran_CD45.txt*.

```{r}
tmp = scData.combined@meta.data
tmp$cell_id = rownames(tmp)
write.table( tmp , file="Metadata_complete_Ramachandran_CD45.txt", sep="\t", row.names = F, quote = F)
```
...
```{r}
outfile = file.path(result_folder,"Cluster_markers_NAFLD_Ramachandran_CD45_annotated.xlsx")
