---
title: "Cluster identification - BCN NAFLD and complete Ramachandran (CD45+ and CD45-)"
...
```{r}
load(file.path(data_storage_folder, "BCN_NAFLD_and_Ramachandran_complete.combined.RData"))
...
## Relation between new clusters and clusters in BCN-NAFLD/Ramachandran CD45+ analysis

```{r}
BCN_NAFLD_meta = read.table("./Metadata_complete_Ramachandran_CD45.txt", sep="\t", header=T, stringsAsFactors = F)
rownames(BCN_NAFLD_meta) = BCN_NAFLD_meta$cell_id

#scData.combined@meta.data$clusters_BCN_NAFLD = NA
scData.combined@meta.data$clusters_BCN_NAFLD = BCN_NAFLD_meta[Cells(scData.combined), "cluster_label"]
```


```{r}
tt = table(scData.combined$seurat_clusters, scData.combined$clusters_BCN_NAFLD, dnn=c("New clusters", "BCN NAFLD/R.CD45+ cluster labels"))
tt
```
...
```{r}
cluster_assignment = read.table("Cluster_assignments/BCN_NAFLD_Clusters_with_Ramachandran_complete_2022-02-07.txt", sep="\t",header=T, stringsAsFactors = F)
options(width = 180)
...
mm_rel_ts$Tissue = ifelse(grepl("Healthy|NALFD", mm_rel_ts$Group), "Adipose tissue","Liver")
...
sel_genes = toupper(c("Cd68","Csf1r","Cx3cr1","Cd14","FCGR3A","Itgam","Ccr2","Adgre4","Adgre1","Fcgr1","Spn","Ly6c","Ly6a", "Vim","CK19"))
...
```{r}
save(scData.combined,file=file.path(data_storage_folder, "BCN_NAFLD_Ramachandran_complete.combined_cluster_annotated.RData"))
```
...
if(rerun_analyses) {
  ...
  save(cluster_markers, file=file.path(data_storage_folder, "All_markers_BCN_and_Ramachandran_complete.Rdata"))
  write_xlsx(cluster_markers,file.path(result_folder, "All_markers_BCN_and_Ramachandran_complete.xlsx"))
} else {
  load(file.path(data_storage_folder, "All_markers_BCN_and_Ramachandran_complete.Rdata"))
}
```
...
DefaultAssay(scData.combined) <- "RNA"
  ...
  save(cons_markers, file=file.path(data_storage_folder,"Conserved_cluster_markers_BCN_and_Ramachandran_complete.Rdata"))
  ...
  write_xlsx(cons_markers_all,file.path(result_folder, "All_cons_markers_BCN_and_Ramachandran_complete.xlsx"))
  
} else {
  load(file.path(data_storage_folder,"Conserved_cluster_markers_BCN_and_Ramachandran_complete.Rdata"))
}
...
project_name = "BCN_and_Ramachandran_complete"
...
Cluster assignations have been exported to file *Metadata_complete_Ramachandran_complete.txt*.

```{r}
tmp = scData.combined@meta.data
tmp$cell_id = rownames(tmp)
write.table( tmp , file="Metadata_complete_Ramachandran.txt", sep="\t", row.names = F, quote = F)
```


## Cluster markers - annotated

Annotated cluster markers have been stored in file *`r file.path(result_folder,"Cluster_markers_NAFLD_Ramachandran_copmlete_annotated.xlsx")`*


```{r}
outfile = file.path(result_folder,"Cluster_markers_NAFLD_Ramachandran_complete_annotated.xlsx")
