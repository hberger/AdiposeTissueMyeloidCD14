---
title: "Cluster identification - Myeloid cells only"
...
```{r}
if (preprocess_data) {
  load(file.path(data_storage_folder, "BCN_NAFLD.combined_cluster_annotated.RData"))

  set.seed(1234)
  
  sc.list <- SplitObject(subset(scData.combined, cluster_type %in% c("DC","MoMF","KC")), split.by = "batch")
  ...
  save(scData.combined, file=file.path(data_storage_folder, "BCN_NAFLD_MoMF_DC_only.combined.RData"))
} else {
  load(file.path(data_storage_folder, "BCN_NAFLD_MoMF_DC_only.combined.RData"))
}
...
## Relation between new clusters and clusters in global analysis

```{r}
table(scData.combined$seurat_clusters, scData.combined$cluster_label, dnn=c("New clusters", "Global cluster labels"))
```
...
if(rerun_analyses) {
  ...
  save(cluster_markers, file=file.path(data_storage_folder, "All_markers_MoMF_DC_reprocessed.Rdata"))
  write_xlsx(cluster_markers,file.path(result_folder, "All_markers_MoMF_DC_reprocessed.xlsx"))
} else {
  load(file.path(data_storage_folder, "All_markers_MoMF_DC_reprocessed.Rdata"))
}
```
...
```{r}
cluster_assignment = read.table("Cluster_assignments/BCN_NAFLD_Clusters_MoMF_DC_2021-11-15.txt", sep="\t",header=T, stringsAsFactors = F)
options(width = 180)
...
```{r}
save(scData.combined,file=file.path(data_storage_folder, "BCN_NAFLD_MoMF_DC_only.combined_cluster_annotated.RData"))
```
...
write.table( tmp , file="Metadata_complete.txt", sep="\t", row.names = F, quote = F)
```
